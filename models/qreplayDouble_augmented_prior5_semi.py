    ...
    def state_creator(self,state):
            ...
            if (cols,rows) not in [self.exit_cell]:
                if np.sum(np.abs(move)) == 2:
                    move_l = (move[0],0)
                    move_r = (0,move[0])
                    move_l2 = (2*move[0], move[1])
                    move_r2 = (move[0], 2*move[1])
                ...
                ray_moves = [move_l, move_l2, move, move_r2, move_r]
                
                for action in ray_moves:
                    for i_dist in range(1,self.maze.shape[0] + 1):
                        cell = (cols + i_dist*action[0], rows + i_dist*action[1])
                        if flags.get(action,0) == 0 and (cell[0] < 0  or cell[0] > self.maze.shape[0] - 1 \
                            or cell[1] < 0 or cell[1] > self.maze.shape[1] - 1 or (cell in self.cells and self.maze[cell[::-1]] == 1)):
                            obst[action] = (i_dist - 1)*norm(action)
                            flags[action] = 1
...
class QReplayDoubleAugmPrior5(AbstractModel):
