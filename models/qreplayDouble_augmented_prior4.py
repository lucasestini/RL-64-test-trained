    ...
    def state_creator(self,states):
        ...
        for i_state in range(len(states)):
            ...
            for i in range(self.maze.shape[0]):
                if right_flag == 0 and ((cols + 1 + i,rows) in self.walls or ((cols + 1 + i, rows) in self.cells and self.maze[(cols + 1 + i, rows)[::-1]] == 1 or cols == self.maze.shape[1]-1)):
                    obst_right = i
                    right_flag = 1
                if left_flag == 0 and ((cols - 1 - i, rows) in self.walls or ((cols - 1 - i, rows) in self.cells and self.maze[(cols - 1- i, rows)[::-1]] == 1 or cols == 0)):
                    obst_left = i
                    left_flag = 1
                if down_flag == 0 and ((cols, rows + 1 + i) in self.walls or ((cols, rows + 1 + i) in self.cells and self.maze[(cols, rows + 1 + i)[::-1]] == 1 or rows == self.maze.shape[0] -1)):
                    obst_down = i
                    down_flag = 1
                if up_flag == 0 and ((cols, rows - 1 - i) in self.walls or ((cols, rows - 1 - i) in self.cells and self.maze[(cols, rows - 1 - i)[::-1]] == 1 or rows == 0)):
                    obst_up = i
                    up_flag = 1
            target_delta_col = self.exit_cell[0] - cols           
            target_delta_row = self.exit_cell[1] - rows
            state_augm[i_state] = np.array([[*(obst_left, obst_right, obst_up, obst_down, target_delta_col, target_delta_row)]])
        return state_augm



class QReplayDoubleAugmPrior4(AbstractModel):
        ...
        for episode in range(1, episodes + 1):
            ...
            while True:
                if np.random.random() < exploration_rate:
                    ...
                    action_index = actions_dict.get(action)
