    ...
    def get_samples(self, sample_size=10):
        ...
        for i, idx in enumerate(np.random.choice(range(mem_size), sample_size, replace=False)):
            ...
            if status == "win":
                ...
                targets[i, move] = reward + self.discount * np.max(self.predict_target(next_state))


        return states, targets


class QReplayDouble(AbstractModel):
    ...
    def __init__(self, game, **kwargs):
        ...
        self.target_model.add(Dense(game.maze.size, input_shape=(2,), activation="relu"))
    ...
    def train(self, stop_at_convergence=False, **kwargs):
        ...
        for episode in range(1, episodes + 1):
            ...
            if episode % 100 == 0 or episode == 5:
                Z_m = []
                for i_act in range(4):
                    Z = np.zeros((8,8))
                    for i in range(8):
                        for j in range(8):
                            state = (i,j)
                            if state in self.game.empty:
                                print(self.q(np.array([list(state)])), type(self.q(np.array([list(state)]))))
                                Z[i,j] = self.q(np.array([list(state)]))[i_act]
                ...
                for i_act in range(4):
                    ...
                    ax.plot_surface(X, Y, Z_m[i_act], rstride=1, cstride=1, cmap='viridis', edgecolor='none')
